...
Follow-up клавиша не может быть буквой: `&kp` шлёт голый скан-код физической клавиши, а во что он превратится — решает раскладка ОС, и под русской тот же скан-код превращается в кириллицу, которую herdr (ждёт литеральный ASCII) не узнаёт. Поэтому все herdr-макросы бьют по стрелкам и F5-F10 — их ни одна раскладка не трогает. F1-F4 без модификатора не годятся: терминал шлёт их формой SS3 (`ESC O P..S`), herdr её не разбирает, и бинд молча не срабатывает (с модификатором те же клавиши идут как CSI и работают — отсюда `shift+f4` у `rename_tab`). Цифры заняты под `switch_tab` в самом herdr, не переиспользовать.
